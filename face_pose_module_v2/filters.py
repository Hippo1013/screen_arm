...
class PoseStabilizer:
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.ema_alpha_axis = float(config.get("ema_alpha_axis", 0.25))
        self.window = max(1, int(config.get("moving_average_window", 1)))
        self.center_deadband_m = float(config.get("center_deadband_m", 0.01))
        self.normal_deadband_deg = float(config.get("normal_deadband_deg", 1.5))
        self.x_axis_deadband_deg = float(config.get("x_axis_deadband_deg", 1.5))
        self.hold_seconds = float(config.get("hold_last_valid_seconds", 0.35))
