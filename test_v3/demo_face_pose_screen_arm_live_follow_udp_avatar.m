...
fig = figure( ...
    "Name", "Live Face Pose Screen Arm Follow UDP v3 Avatar", ...
    "NumberTitle", "off", ...
    "Color", "w", ...
    "MenuBar", "none", ...
    "ToolBar", "figure", ...
...
pythonLogFile = fullfile(tempdir, "screen_arm_face_pose_live_udp_v3.log");
...
processLatestFacePose(fig, true);
end

function didUpdate = processLatestFacePose(fig, allowPlanning)
didUpdate = false;
if ~isvalid(fig)
    return
end
...
state = guidata(fig);
if allowPlanning && state.following && ~state.isMoving && targetChangeNeedsMove(state)
...
commandFaceNormal = state.faceNormalWorld;
commandTargetPoint = nominalTargetPoint(state);
...
if solveResult.reachable
    ...
    state.targetReachable = true;
    state.targetDistance = solveResult.distance;
    state.lastCommandFaceNormal = commandFaceNormal;
    state.lastCommandTargetPoint = commandTargetPoint;
...
for frameIndex = 1:frameCount
    ...
    while toc(frameTimer) < secondsPerRobotFrame
        ...
        processLatestFacePose(fig, false);
        drawnow limitrate
